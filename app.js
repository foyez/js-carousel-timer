...
const left = () => {
	if (translationComplete) {
		...
		images.forEach((img, i) => {
			img.style.opacity = 1;
			img.style.transform = 'translate(' + (currTransl[i] - size) + 'px)';
			currTransl[i] = currTransl[i] - size;
		});

		var outerIndex = (index - 1) % amount;

		var outerImg = document.querySelectorAll('.slide')[outerIndex];
		console.log(currTransl[outerIndex] + size * amount);
		outerImg.style.transform =
			'translateX(' + (currTransl[outerIndex] + size * amount) + 'px)';
		outerImg.style.opacity = '0';
		currTransl[outerIndex] = currTransl[outerIndex] + size * amount;
	}

	setTimeout('left()', 3500);
};
